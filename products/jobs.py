...
def send_email(upload_job_id, order_id, email):
    ...
    r = send_mail('Your CipheRing is now ready to order!', 'Woohoo! Your CipheRing is now ready to order at this URL: {0}'.format('{0}/#/order/{1}?from=email'.format(settings.FRONTEND_BASE_URL, order.uuid)), 'CipheRing <admin+ciphering@pb.io>', [email], fail_silently=False)
